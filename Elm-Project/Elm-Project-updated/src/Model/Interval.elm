...
Intervals are first compared compare by tIMhe `start` field.
        ...
        EQ -> 
            case (intA.end, intB.end) of
                (Nothing, Nothing) -> 
                    EQ
                (Just endA, Just endB) ->
                    case Date.compare endA endB of
                        EQ ->
                            EQ
                        order ->
                            order
    ...
    let
        endText = 
            case interval.end of
                Just end ->
                    div [class "interval-end"] [Date.view end]
                Nothing ->
        ...
        lengthText =
            case length (Interval interval) of
                ...
                Nothing ->
                    div [class "interval-length"] [text " "] 
    in
        div [class "interval"]
