...
monthsBetween (Date dA) (Date dB) = 
    case (dA.month, dB.month) of
        (Just monthA, Just monthB) ->
            let
                years = abs (dA.year - dB.year)
                months = abs((monthToInt monthA) - (monthToInt monthB))
                result_months = 
                    if years == 0 then 
                        months
                    else if years == 1 then
                        if compare (Date dA) (Date dB) == GT then
                            (12 - (monthToInt monthB)) + (monthToInt monthA)
                        else
                            (12 - (monthToInt monthA)) + (monthToInt monthB)
                    else 
                        if compare (Date dA) (Date dB) == GT then
                            years * 12 - months
                        else 
...
view (Date d) =
    let
        theYear = String.fromInt d.year
        theMonth = d.month |> Maybe.map monthToString |> Maybe.withDefault ""
    in
         div [] [ text (theYear ++ " " ++ theMonth)]
